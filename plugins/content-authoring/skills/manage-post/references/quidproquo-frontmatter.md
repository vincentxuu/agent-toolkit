...
| `category` | string | ✅ | 分類資料夾名，以目前 `check-post-quality` validator 清單為準 |
| `tags` | string[] | ✅ | 標籤陣列；可空 `[]`，但通常 3-7 個 |
| `lang` | `'zh-TW'` \| `'en'` | ✅ | validator 要求明填；英文版使用 `-en.md` 與 `/posts/...-en` route |
| `updated` | date（coerced） | ❌ | 最後一次實質修訂日期 |
...
| `additionalSeries` | object[] | ❌ | 額外所屬系列，每筆含 `name`、`order` |
| `relatedDigests` | object[] | ❌ | 關聯 digest，每筆含 `slug`、`reason` |
| `faq` | object[] | ❌ | FAQ 結構，每筆含可獨立閱讀的 `q` 與 `a` |
