...
- 核心原則與各分類風格
- Title、tldr、description、tags 與語氣
- 台灣用語、翻譯腔與兩層檢查
- 長文的體裁與語域
- AI 搜尋友善（GEO）
- 參考資料與 inline 連結
...
分兩級：**A 級**是台灣沒有正當用法的詞（例如用戶、視頻、插件），一定要改；**B 級**看語境（例如機器學習的「激活」、物理的「質量」、專有名詞裡的「智能」、控制理論的「反饋」），自己判斷。詞表刻意窄——寧可漏抓也不要有噪音，紅燈才能被信任。要加詞改 `scripts/check-tw-terms.mjs` 頂端的 `BLOCK` / `WARN` 兩張表。
...
- [ ] 比較表第一欄的論文 / 工具名稱都有連結
- [ ] `pnpm verify` 全綠（包含 references、post quality、TW、glossary、lang parity 等 fast checks）
- [ ] `pnpm astro check` 或目前專案指定的 content schema/build check 全綠
