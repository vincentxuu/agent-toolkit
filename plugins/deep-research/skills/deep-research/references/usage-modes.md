...
Bearer token 應存在平台的 connector secret storage，不貼進對話。若平台沒有 custom remote MCP 能力，或本次 session 未暴露 Groundlane tools，使用平台可用的原生 search/fetch/browser 或其他專用 connector，並標示 fallback；不要聲稱已使用 Groundlane。
