...
- 「要直接用 `manage-post` skill 建立或更新導讀文嗎？」（→ 選 create/update；目標是 Quidproquo 時套用其站點規則）
- 「還想補哪些子問題？」
- 「先存著之後再發？」
...
- **deep-research → manage-post**：若環境已安裝 `manage-post` skill，研究完可把草稿骨架交給它建立新文或更新既有文章；未安裝時直接交付 research note
...
- 發文交接：若已安裝 `manage-post` skill，載入並遵循該 skill 自己公開的流程；不要假設其內部檔案結構
